...
const sheetVariants: Record<string, string> = {
  ...
  right: "inset-y-0 right-0 h-full w-full max-w-[720px] border-l data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right",
};

const SheetContent = React.forwardRef<
  React.ComponentRef<typeof DialogPrimitive.Content>,
  SheetContentProps & { preventOverlayClose?: boolean }
>(({ side = "right", className, children, preventOverlayClose, ...props }, ref) => (
  <SheetPortal>
    {/* When preventOverlayClose is true, don't render the overlay so it can't block clicks */}
    {!preventOverlayClose && <SheetOverlay />}
    <DialogPrimitive.Content
      ref={ref}
      onInteractOutside={preventOverlayClose ? (e) => e.preventDefault() : undefined}
